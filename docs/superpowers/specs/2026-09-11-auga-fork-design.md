...
1. Known breakages (from the static check):
   ...
   - Decision (2026-09-11, user): the main menu stays vanilla. Auga's main-menu replacement is dropped. Only the font of Auga's old menu buttons (bundle TMP `Norsebold SDF`) goes on the vanilla button labels (body/list texts stay vanilla), and the 1.0.7 Cinematics button is hidden. In-game UI (HUD, inventory, etc.) stays Auga.
